...
#' @param error a numeric vector of standard error estimates corresponding
...
fmt_effect <- function(
  ...
  if(!is.null(conf_level)){
    ...
    output <- paste0(
      ...
      adapt_round(fun(effect + qnorm(alpha / 2) * error)), ", ",
      adapt_round(fun(effect + qnorm(1 - alpha / 2) * error)), ")"
    )
